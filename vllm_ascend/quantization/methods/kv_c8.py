...
class AscendC8KVCacheAttentionMethod(AscendAttentionScheme):
    ...
    def create_weights(self, layer: torch.nn.Module) -> None:
        ...
        # Full-graph update dispatch keys off layer.attn_backend.get_impl_cls(),
        # so C8 needs a dedicated backend class, not only impl class surgery.
        if hasattr(layer, "impl") and hasattr(layer, "attn_backend"):
            from vllm_ascend.attention.attention_v1 import (
                AscendC8AttentionBackend,
                AscendC8AttentionBackendImpl,
            )

            layer.impl.__class__ = AscendC8AttentionBackendImpl
            layer.attn_backend = AscendC8AttentionBackend
